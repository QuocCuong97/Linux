# Sử dụng lệnh `virt-install` để tạo máy ảo
## **1) Giới thiệu**
- `virt-install` là công cụ command-line dùng cho việc tạo máy ảo trên KVM sử dụng thư viện quản lí "`libvirt`".
- `virt-install` hỗ trợ cả dạng text lẫn đồ họa, người sử dụng có thể lựa chọn giữa VNC - SDL cho chế độ cài đồ họa hoặc text console. Bên cạnh đó, cũng có thể lựa chọn cài đặt dùng 1 hoặc nhiều các thành phần như virtual disks, card mạng, ...
...
## **2) Các options khi tạo máy ảo với `virt-install`**
...
- Các tùy chọn boot :
    ...
    - `-l` hoặc `--location=LOCATION` : Đường dẫn tới file cài đặt từ trên internet . Một số đường dẫn file cài đặt từ Internet :
        - **CentOS 7** :
        - **Ubuntu 18.04** :
    ...
    - `--os-variant=OS_VARIANT` : Là một optional command, cho biết xa hơn về loại hệ điều hành . Sử dụng `osinfo-query os` để xem tất cả list os . Một số các os variant thông dụng :
...
- Các tùy chọn lưu dữ liệu :
    - `--disk=DISKOPTS` : Thiết lập nơi lưu dữ liệu của máy ảo. Format chung: `--disk tuychon1=giatri1,tuychon2=giatri2,...` .Các tùy chọn đi kèm :
...
- Các tùy chọn về ảo hóa :
    - `-v` hoặc `--hvm` : Ảo hóa toàn bộ
    - `-p` hoặc `--paravirt` : Ảo hóa song song
    - `--virt-type` : Chọn hypervisor sẽ cài đặt lên (KVM, QEMU hoặc XEN...)
